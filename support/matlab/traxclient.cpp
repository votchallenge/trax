...
command call_callback(const mxArray *callback, status& s, const mxArray* data) {
	...
	const char* fieldnames[] = {"region", "time", "properties"};
	mxArray * st = mxCreateStructMatrix(1, 1, 3, (const char **) fieldnames);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	const mxArray* data = mxCreateDoubleMatrix( 0, 0, mxREAL );
    ...
    if ( nrhs > 1 ) {
		...
		for (int i = 2; i < std::floor((float)nrhs/2) * 2; i+=2) {
			...
			//case ARGUMENT_ENVIRONMENT: environment = (int) mxGetScalar(prhs[i+1]); break;
			case ARGUMENT_STATEOBJECT: data = prhs[i+1]; break;
			default:
