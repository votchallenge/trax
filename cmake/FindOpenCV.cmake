...
IF(UNIX)
LIST(APPEND __SEARCH_DIRS /usr/local /usr/)
SET(__SUFFIXES include/opencv)
ELSE(UNIX)
LIST(APPEND __SEARCH_DIRS "C:/Work/C/OpenCV2.0" ENV{OpenCV_DIR})
SET(__SUFFIXES "vs2008" "vs2009" "include/opencv")
ENDIF(UNIX)


FOREACH(__DIR ${__SEARCH_DIRS})

    FIND_PATH(OpenCV_CONFIG "cv.h" PATHS ${__DIR} PATH_SUFFIXES ${__SUFFIXES} NO_DEFAULT_PATH)
    IF (EXISTS "${OpenCV_CONFIG}")
        SET(OpenCV_DIR ${__DIR})
        BREAK()
    ENDIF (EXISTS "${OpenCV_CONFIG}")

ENDFOREACH(__DIR)

#MESSAGE(STATUS "${OpenCV_CONFIG} ${OpenCV_DIR} - ${__SEARCH_DIRS}")
...
if(EXISTS "${OpenCV_DIR}")
        ...
        if(EXISTS "${OpenCV_CONFIG}/OpenCVConfig.cmake")
                ...
                IF(UNIX)
                LIST(APPEND CMAKE_FIND_LIBRARY_SUFFIXES .so.${OpenCV_VERSION_MAJOR}.${OpenCV_VERSION_MINOR}.${OpenCV_VERSION_PATCH})
                set(CVLIB_SUFFIX "")
                ELSE(UNIX)
                set(CVLIB_SUFFIX "${OpenCV_VERSION_MAJOR}${OpenCV_VERSION_MINOR}${OpenCV_VERSION_PATCH}")
                ENDIF(UNIX)
        #Otherwise it try to guess it.
        else(EXISTS "${OpenCV_CONFIG}/OpenCVConfig.cmake")
        ...
        endif(EXISTS "${OpenCV_CONFIG}/OpenCVConfig.cmake")
        ...
        foreach(__CVLIB ${OPENCV_LIB_COMPONENTS})
                ...
                find_library(OpenCV_${__CVLIB}_LIBRARY_DEBUG NAMES "${__CVLIB}d" "${__CVLIB}${CVLIB_SUFFIX}d" PATHS "${OpenCV_DIR}/lib" NO_DEFAULT_PATH)
                find_library(OpenCV_${__CVLIB}_LIBRARY_RELEASE NAMES "${__CVLIB}" "${__CVLIB}${CVLIB_SUFFIX}" PATHS "${OpenCV_DIR}/lib" NO_DEFAULT_PATH)
                ...
                # Cleanup a bit ...
                UNSET(OpenCV_${__CVLIB}_LIBRARY_DEBUG CACHE)
                UNSET(OpenCV_${__CVLIB}_LIBRARY_RELEASE CACHE)
#MESSAGE("${OpenCV_${__CVLIB}_LIBRARY}")
